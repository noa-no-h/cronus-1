...
export const SelectionBox: React.FC<SelectionBoxProps> = ({ isVisible, dragState, yToTime }) => {
  ...
  const height = Math.abs(start.y - end.y) - 3

  return (
    <div
      className="absolute left-[68px] mr-1 my-1 right-0 bg-blue-500/30 border-2 border-blue-500 rounded-md z-10 pointer-events-none transition-all duration-100 ease-out"
      style={{
        top: `${top}px`,
        height: `${height}px`
      }}
