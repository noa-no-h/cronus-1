...
export async function fetchWatchers(owner: string, repo: string): Promise<string[]> {
  const watchers: string[] = [];
  let page = 1;
  while (true) {
    try {
      const url = `https://github.com/${owner}/${repo}/watchers?page=${page}`;
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; PrimeIntellectBot/1.0;)',
        },
      ...
      if (!response.ok) {
        console.error(`Error fetching watchers page ${page}: ${response.status}`);
        break;
      }
      ...
      const userElements = Array.from(
        document.querySelectorAll('.follow-list-item .f4 a.d-inline-block')
      );

      if (userElements.length === 0) {
        break; // No more watchers found
      }
      ...
      console.error(`Error scraping watchers page ${page}:`, error);
      break;
    }
  }
  return watchers;
